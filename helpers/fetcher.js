...
const exec = ({
  ...
  return client
    ...
    .then(
      res => after(values, res).then((converted) => {
        dispatch(
          success({
            body: converted,
            res,
...
const getExecOptions = (
  applyWith,
  {
    options, values, urls, dispatch, client, resource, action, type,
  },
) => ({
  dispatch,
  client,
  ...
  cache: urls[resource][action].cache && urls[resource][action].cache[type],
  mode: options.mode !== undefined ? options.mode : urls[resource][action].mode,
  credentials:
    options.credentials !== undefined ? options.credentials : urls[resource][action].credentials,
  ...
  success: ({ body, res, next }) => {
    ...
    return {
      ...
      hasNext: !!(urls[resource][action].next && next),
    };
  },
  fail: ({ body, res, err }) => ({
    values,
...
const generateNext = (applyWith, {
  action, options, resource, urls, next, dispatch, client,
}) => {
  if (!next) {
    return Promise.resolve();
  }
  ...
  return exec({
    ...
    success: ({ body, res, next: _next }) => {
      ...
      return {
        ...
        hasNext: !!(urls[resource][action].next && _next),
      };
    },
    fail: ({ body, res, err }) => ({
      values,
...
const deserialize = (applyWith, {
  serialized, resource, action, urls, dispatch, client,
}) => {
  if (
    serialized
    && serialized[resource]
...
export default class fetcher {
  constructor({
    urls, dispatch, client, serialized, type,
  }) {
    Object.keys(urls).map((resource) => {
      ...
      Object.keys(urls[resource]).map((action) => {
        this[resource][action] = (_values, options = {}) => {
          ...
          const execOptions = getExecOptions(this, {
            options,
            values,
            urls,
            dispatch,
