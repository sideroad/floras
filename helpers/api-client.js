...
const string = values => __.map(values, (value, key) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join(
  '&',
);
    ...
    this.fetchJSON = ({
      ...
      return new Promise((resolve, reject) => {
        ...
        fetch(
          _url,
          method === 'GET'
            ? getHeader(headers, defaultHeaders, mode, credentials)
            : method === 'POST'
              ? postHeader(_values, headers, defaultHeaders, mode, credentials, useQuery)
              : method === 'PATCH'
                ? patchHeader(_values, headers, defaultHeaders, mode, credentials, useQuery)
                : method === 'DELETE'
                  ? deleteHeader(_values, headers, defaultHeaders, mode, credentials, useQuery)
                  : '',
        ).then(
          (res) => {
            if (!res.ok) {
