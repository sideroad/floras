...
export function cachedRequest(apiUrl) {
...
export function remove({ place, type }) {
  return request(
    'DELETE',
        ...
        scripts: ['http://code.jquery.com/jquery.js'],
          ...
          const spots = $('.list01 a')
            .map((index, elem) => {
              ...
              const date = moment(
                `${dateMatched[1]}/${dateMatched[2]}/${dateMatched[3]}`,
                'YYYY/M/D'
              ).format('YYYY-MM-DD');
              const shots = $this
              ...
              return {
                ...
                strength:
                  (shots ? Number(`${!shots[2] ? shots[1] * 10000 : shots[1]}${shots[2]}`) : 2000) /
                  50,
              };
            })
            .get();
...
export function crawl(season, evaluator, type) {
  return new Promise((resolve) => {
